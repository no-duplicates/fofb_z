function [P, G, C] = ofbmdl_2k(M, Mc,Mc_local, K_1,K_2, A, F, Wd, Wz, Wn)
% OFBMDL Orbit Feedback Model.
% 
% Construct orbit feedback state-space model.
%
% [P, G, C] = OFBMDL(M, Mc, K_1,K_2, A, F, Wd, Wz, Wn)
%
% INPUTS:
...
%   K_1:    Controller gain (scalar or array with NCORR elements).
...
if nargin < 8 || isempty(Wd)
    Wd = eye(nbpms);
end

if nargin < 9 || isempty(Wz)
    Wz = eye(nbpms);
end

if nargin < 10 || isempty(Wn)
    Wn = eye(nbpms);
end
...
% The delay in A has been reduced by 1 Ts
...
z = tf('z', Ts);
one_ts_delay = ss(1/z);
C = ss(F)*diag(K_1)*Mc_local*tf([Ts 0], [1 -1], Ts)+one_ts_delay*ss(F)*diag(K_2)*(Mc-Mc_local)*tf([Ts 0], [1 -1], Ts);
